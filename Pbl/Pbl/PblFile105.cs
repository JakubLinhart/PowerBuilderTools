	...
	public sealed class PblFile105 : PblFile {
		...
		private void LoadLibraryHeader() {
			...
			if (hdr != PblFile105.BLOCK_NAME_HDR)
				throw new PblFileException("Wrong PBL signature. Signature " + PblFile105.BLOCK_NAME_HDR + " is expected.");

			// signature: "PowerBuilder\0\0"
			hdr = reader.ReadUnicodeString(VERSION_SIGANTURE.Length);
			if (hdr != PblFile105.VERSION_SIGANTURE)
				throw new PblFileException("Wrong PBL signature. Signature " + PblFile105.VERSION_SIGANTURE + " is expected.");
		...
		private PblNod LoadNod(long offset) {
			...
			// is it really a NOD section?
			string hdr = reader.ReadBlockName();
			if (hdr != PblFile105.BLOCK_NAME_NOD)
                throw new PblFileException("PBL corrupted. ENT section was not found.");

			int offset_previous = reader.ReadInt32(); // prev NOD
			int offset_parent = reader.ReadInt32(); // parent NOD ???
			int offset_next = reader.ReadInt32(); // next NOD
			int unknown = reader.ReadInt32(); // ???????
			short entries_count = reader.ReadInt16(); // ENT (entries) count in this NOD
