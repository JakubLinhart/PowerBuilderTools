	...
	public sealed class PblFile105 : PblFile {
		...
		private void LoadLibraryHeader() {
			...
			if (hdr != PblFile105.BLOCK_NAME_HDR)
				throw new PblFileException("Chybný zaèátek souboru PBL knihovny. Je oèekávána signatura " + PblFile105.BLOCK_NAME_HDR);

			// signatura: "PowerBuilder\0\0"
			hdr = reader.ReadUnicodeString(VERSION_SIGANTURE.Length);
			if (hdr != PblFile105.VERSION_SIGANTURE)
				throw new PblFileException("Chybný zaèátek souboru PBL knihovny. Je oèekávána signatura " + PblFile105.VERSION_SIGANTURE);
		...
		private PblNod LoadNod(long offset) {
			if (offset > reader.BaseStream.Length)
				throw new PblFileException("Chyba v souboru PBL knihovny. Soubor neobsahuje všechny NOD sekce.");
