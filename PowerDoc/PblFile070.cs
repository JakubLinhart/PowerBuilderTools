	...
	public sealed class PblFile070 : PblFile {
		...
		private void LoadLibraryHeader() {
			...
			if (hdr != PblFile070.BLOCK_NAME_HDR)
				throw new PblFileException("Chybný zaèátek souboru PBL knihovny. Je oèekávána signatura " + PblFile070.BLOCK_NAME_HDR);

			// signatura: "PowerBuilder\0\0"
			hdr = reader.ReadASCIIString(VERSION_SIGANTURE.Length);
			if (hdr != PblFile070.VERSION_SIGANTURE)
				throw new PblFileException("Chybný zaèátek souboru PBL knihovny. Je oèekávána signatura " + PblFile070.VERSION_SIGANTURE);
